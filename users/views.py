    ...
    def get_queryset(self):
        ...
        if filter_type and self.request.user.is_authenticated:
            if filter_type == 'owners-of-favorite-projects':
                queryset = User.objects.filter(owned_projects__favorites=self.request.user).distinct()
            elif filter_type == 'owners-of-participating-projects':
                queryset = User.objects.filter(owned_projects__participants=self.request.user).distinct()
            elif filter_type == 'interested-in-my-projects':
                queryset = User.objects.filter(favorite_projects__owner=self.request.user).distinct()
            elif filter_type == 'participants-of-my-projects':
                queryset = User.objects.filter(participating_projects__owner=self.request.user).distinct()
